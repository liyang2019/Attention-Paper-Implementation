...
class TestContextInnerProd(unittest.TestCase):
  ...
  def test_context_inner_prod(self):
    ...
    context_inner_prod = ContextInnerProd(encoder_hidden_size, hidden_size, context_size=context_size)
    decoder_hidden = Variable(torch.FloatTensor(np.random.randn(1, encoder_hidden_size)))
